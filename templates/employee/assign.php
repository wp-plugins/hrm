...
<div id="hrm-assign-leave-summary" class="hrm-assign-leave">
    ...
    if( $search_status ) {
        ...
        update_user_meta( get_current_user_id(), '_hrm_search_data',
            array( 'table_option' => 'hrm_leave', 'emp_id' => $employer_id, 'type_id' => $search_post['type_id'], 'start_time' => $search_post['start_time'], 'end_time' => $search_post['end_time'] ) );
    } else{
        $get_leave_users = Hrm_Settings::getInstance()->conditional_query_val( 'hrm_leave', array( 'DISTINCT emp_id' ), array( 'emp_id' => $employer_id ), false );
